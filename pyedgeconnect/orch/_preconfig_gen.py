...
#    info = orch.get("/appliance/extraInfo/" + nepk).json()
    info = self.appliance_extra_info(nepk).json()  
...
def deployment(self, nepk: str, labels, zones):
    yaml_text = ""
    dhcp_yaml_text = ""
    dhcpInfo = False
#RBAC may need the ?source=menu_deployment_id
    dep = self.appliance_get_api(nepk, "/deployment").json()
...
def routes(self, nepk):
    yaml_text = "" 
    sys = self.appliance_get_api(nepk , "/system").json()
    yaml_text += "\nlocalRoutes: \n" 
    ...
    try:
        sub3 = self.appliance_get_api(nepk, "/subnets3/configured").json()
...
def loopback(self, nepk, labels, zones):
    yaml_text = ""
    loopback = self.appliance_get_api(nepk, "/virtualif/loopback").json()
...
def bgp(self, nepk):
    yaml_text = ""
    bgp = self.appliance_get_api(nepk, "/bgp/config/system").json()
    bgp_neigh = self.appliance_get_api(nepk , "/bgp/config/neighbor").json()
    if(bgp['enable']):
        yaml_text += "\nbgpSystemConfig: \n"
...
def templates(self, nepk):
    yaml_text = ""
    templates = self.get_appliance_template_groups_association(nepk).json()
    yaml_text += "\ntemplateGroups: \n"
    yaml_text += "  groups: \n"
...
def bio(self, nepk):
    yaml_text = ""
    bio = self.get_all_appliance_overlay_association().json()
    bio_config = self.get_all_overlays_config().json()
    bio_dict = {}
...
def inbound_port_forwarding(self, nepk):
    yaml_text = ""
    ipf = self.get_appliance_port_fowarding("/portForwarding/" + nepk + "?source=menu_deployment_id").json()
    if(ipf):
